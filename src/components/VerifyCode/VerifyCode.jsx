...
export default function VerifyCode() {
    ...
    async function handleVerifyCode(values) {
        try {
            ...
            setErrorMsg("the code isn't valid")
        }
        finally {
            setIsLoading(false)
        }
    ...
    const yupSchema = Yup.object().shape({
        resetCode: Yup.string().required("Verify Code is a required field")
    });
    ...
    <div>
        <h2 className='text-start text-2xl mb-2'>Reset your account password</h2>
    </div>
    ...
    <form onSubmit={formic.handleSubmit} className="   w-full gap-5 mb-20">
        ...
        <div className="relative z-0 w-full mb-5 group">
            ...
            <input onChange={formic.handleChange} onBlur={formic.handleBlur} value={formic.values.resetCode} type="text" name="resetCode" id="resetCode" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-green-500 focus:border-green-500 block w-full  p-2.5" required />
        </div>
        {
            formic.errors.resetCode &&
